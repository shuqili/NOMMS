...
l6.taxa %>%  
  ...
  ungroup() %>%
  mutate(feature_pos = c(1:14)) %>% # count and assigne position by feature.site
...
getwd()
ggsave("~/Partners HealthCare Dropbox/Shuqi Li/CL-04-NOMMS/03b-EbioMedicine-Revise/00-github repository/03-differential analyses/06-visualization/08-females-EDSS-change/l6-female-edss-change.pdf",
...
l6.taxa %>%  
  ...
  theme(axis.title = element_blank(),
        ...
        legend.text = element_text(face = "bold", hjust = 0.5))
getwd()
ggsave("/Users/sl/Partners HealthCare Dropbox/Shuqi Li/CL-04-NOMMS/03b-EbioMedicine-Revise/00-github repository/03-differential analyses/06-visualization/09-males-EDSS-change/l6-male-edss-change.pdf",
       width = 2, height = 2.4, dpi = 300) 

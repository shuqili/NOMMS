...
nasal <-
  read_tsv("03-output/01-nostril/l6taxa/Smoking_YN + [age + sex + bmi ]/cplmfit-no-transformation-relab-asv-to-diseasestat/all_results.tsv") %>%
  mutate(anatomical_site = "nasal")
gin <-
  read_tsv("03-output/02-gums/l6taxa/Smoking_YN + [age + sex + bmi ]/cplmfit-no-transformation-relab-asv-to-diseasestat/all_results.tsv") %>%
  mutate(anatomical_site = "gingival")
oro <-
  read_tsv("03-output/03-throat/l6taxa/Smoking_YN + [age + sex + bmi ]/cplmfit-no-transformation-relab-asv-to-diseasestat/all_results.tsv") %>%

...
read_tsv("09-MaAsLin/01-input/metadata-v5-2yr-EDSS-50subjectPool.tsv") %>%
  ...
  filter(sex == "Females") %>%
  write_tsv("../01-NOMMS_Final_qiime_v202309/09-MaAsLin/01-input/metadata-v5-2yr-EDSS-37femalesPool-gums.tsv")





df_input_metadata = read.table(file             = glue("09-MaAsLin/01-input/metadata-v5-2yr-EDSS-37femalesPool-{Anatomical_Location}.tsv"), 
                               header           = T, 
                               sep              = "\t", 
                               row.names        = 1,
...
dir.create("09-MaAsLin/03-output/02-gums/l6taxa/female-edss_change/")

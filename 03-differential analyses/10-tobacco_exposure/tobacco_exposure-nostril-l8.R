...
read_tsv("metadata-CL04-NOMMS-v5.txt") %>%
  ...
  # view
  write_tsv("09-MaAsLin/01-input/metadata-v5-nostrils-tobacco_exposure.tsv")

df_input_metadata = read.table(file             = "09-MaAsLin/01-input/metadata-v5-nostrils-tobacco_exposure.tsv",
                               header           = TRUE, 
                               sep              = "\t", 
                               row.names        = 1,
...
df_input_data = read.table(file             = "09-MaAsLin/01-input/input_ASV_rel_abund.tsv",
                           header           = TRUE,
                           sep              = "\t", 
                           row.names        = 1,
                           stringsAsFactors = FALSE)
...
dir.create("09-MaAsLin/03-output/01-nostril/ASVlevel/tobacco_exposure + [age + sex + bmi ]")
fit_data_4 = Maaslin2::Maaslin2(output = "09-MaAsLin/03-output/01-nostril/ASVlevel/tobacco_exposure + [age + sex + bmi ]/cplmfit-no-transformation-relab-asv-to-diseasestat/", 

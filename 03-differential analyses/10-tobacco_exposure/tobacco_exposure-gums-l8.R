...
read_tsv("metadata-CL04-NOMMS-v5.txt") %>%
  ...
  # view
  write_tsv("09-MaAsLin/01-input/metadata-v5-gums-tobacco_exposure.tsv")

df_input_metadata = read.table(file             = "09-MaAsLin/01-input/metadata-v5-gums-tobacco_exposure.tsv", 
                               header           = TRUE, 
                               sep              = "\t", 
                               row.names        = 1,
...
dir.create("09-MaAsLin/03-output/02-gums/ASVlevel/tobacco_exposure + [age + sex + bmi ]/")
fit_data_4 = Maaslin2::Maaslin2(output = "09-MaAsLin/03-output/02-gums/ASVlevel/tobacco_exposure + [age + sex + bmi ]/cplmfit-no-transformation-relab-asv-to-diseasestat/", 

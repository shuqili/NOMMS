...
anatomicallocation = "Gums"
folder.output <- "02-gums"
...
read_tsv("09-MaAsLin/01-input/metadata-v5-2yr-EDSS-50subjectPool.tsv") %>%
  ...
  filter(sex == "Males") %>%
  write_tsv("../01-NOMMS_Final_qiime_v202309/09-MaAsLin/01-input/metadata-v5-2yr-EDSS-12MalesPool-gums.tsv")
...
dir.create("09-MaAsLin/03-output/02-gums/l6taxa/male-edss_change/")
...
read_tsv("09-MaAsLin/01-input/input_genus_rel_abund.tsv") %>%
  ...
  write_tsv("../03b-EbioMedicine-Revise/00-github repository/03-differential analyses/09-males-EDSS-change/filtered.genus.rel.abund.12males.gums.tsv")
...
df_input_data_trimmed = read.table(file  = "../03b-EbioMedicine-Revise/00-github repository/03-differential analyses/09-males-EDSS-change/filtered.genus.rel.abund.12males.gums.tsv",
                                   header           = TRUE,
                                   sep              = "\t",
                                   row.names        = 1,
                                   stringsAsFactors = FALSE)
